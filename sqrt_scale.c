...
void sqrt_scaling(double* data, long* naxes, double* iscale);
...
void write_ds9_cube(char* outfile, long *naxes, double *iscale, double *ds9_cube, int overwrite);
...
int main(int argc, char **argv){
	...
	sqrt_scaling(data, naxes, iscale);
	get_out_file(out_file, filename, "sqrt_");
	write_ds9_cube(out_file, naxes, iscale, data, 1);
	free(iscale);
...
void sqrt_scaling(double* data, long* naxes, double* iscale){
	...
	puts("applying sqrt scaling");
	for(i=0;i<nelements;i++){
		printf("progrsss: %0.2lf%%\r", 100.0*(double)i/(double)nelements);
		x = (double)(i%naxes[0]);
		//printf("xi: %g", x);
		...
		r = sqrt(x*x + y*y + z*z);
		data[i]/=sqrt(r); //apply square-root scaling
	}
	return;
}
...
void write_ds9_cube(char* outfile, long *naxes, double *iscale, double *ds9_cube, int overwrite){
	...
	if(overwrite){
		printf("Will overwrite any other file with name %s\n", outfile);

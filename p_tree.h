...
#include <float.h>
	...
	//e.g. #define TREE_TYPE double, or #define TREE_TYPE struct particle etc...
#endif
	...
	//e.g. if TREE_TYPE is 'particle', and TREE_SORT is 'pos' then will sort over the 'pos' attribute
#endif
#ifndef N_DIM
...
void qsort_nd2(PARTICLE** data, int N, int dim);
int median_array(PARTICLE **array, int N, int dim);
int order_lt_gt(PARTICLE** array, int m, int N, int dim);
NODE* build_tree_array(PARTICLE **point_array, int N, int K, int depth);
...
double dist2(PARTICLE *a, double *b, int n_dim);
double dist2_d(double *a, double *b, int n_dim);
int any_gt(double* list, double test, int n);
int is_in(NODE** list, NODE* test, int n);
void nearest_nn(NODE* node, NODE** guess, double *g_dist2, PARTICLE* point, int n, int depth, int n_dim);
void nearest_n(NODE* node, NODE** guess, PARTICLE* point, double *g_dist2, int depth, int n_dim);
void create_tree(NODE** root, PARTICLE** data, int N, int K);
void destroy_tree(NODE* node);
void tree_insert(NODE* node, PARTICLE* point, int K, int depth);
void tree_insert_array(NODE* root, PARTICLE** data, int N, int K);
int find_in_array(PARTICLE **point_array, PARTICLE *point, int N);
...
void recompute_nn_dist(NODE **nn_list, double *nn_dist2, double *point, int n, int n_dim){
	...
	for(i=0;i<n;i++){
		...
		nn_dist2[i] = dist2_d(nn_list[i]->val->pos, point, n_dim);
...
void qsort_nd2(PARTICLE** data, int N, int dim){
	...
	PARTICLE *temp;
	if(N<2) return; //if we have 1 or less particles then the list is already sorted
	//printf("In qsort_nd2\narguments: N=%d, dim=%d\n", N, dim);
	//puts("data array:");
	//for(i=0;i<N;i++){
	//	printf("data[%d]->pos[%d] = %G\t%u\n", i, dim, data[i]->pos[dim], data[i]);
	//}
	
	pivot = N/2; //choose middle integer number as the pivot
	piv_id = data[pivot]->ident; //remember the pivot id
	//printf("pivot: data[%d]->pos[%d] = %G \t %u\n", pivot, dim, data[pivot]->pos[dim], data[pivot]);
	
	//j=0;
	for(i=0;i<N;i++){
		//printf("%07d, median %d: %G, test: %G\r", i, pivot, data[pivot]->pos[dim], data[i]->pos[dim]);
		if(data[i]->ident == piv_id)//don't sort the pivot against its self
			continue;
		if((data[i]->pos[dim] < data[pivot]->pos[dim]) && (i > pivot))//if data is smaller than pivot and we are after the pivot
		{
			temp = data[i]; //swap the data and pivot
			data[i] = data[pivot];
			data[pivot] = temp;
			temp_i = pivot; //remember old pivot position
			pivot = i; //our pivot is now in a new position
			i = temp_i; //must check the stuff after the old pivot position again
		}
		else if((data[i]->pos[dim] > data[pivot]->pos[dim]) && (i < pivot))//if data is greater than or equal to pivot and we are before the pivot
		{
			temp = data[i]; //swap the data and pivot
			data[i] = data[pivot];
			data[pivot] = temp;
			pivot = i; //pivot is now at new position, can set off from here as we moved the pivot to earlier in the list
		}
...
int median_array(PARTICLE **array, int N, int dim){
	...
	for(i=N/2; (array[i]->pos[dim]-array[i-1]->pos[dim])==0.0; i--){
		if(i==1){
			i=0;
			break;
		}
...
int order_lt_gt(PARTICLE** array, int m, int N, int dim){
	...
	for(i=0;i<N;i++){
		...
		//printf("%07d %% complete, median at %d: %G, test: %G\r", i, m, array[m]->pos[dim], array[i]->pos[dim]);
		if(array[i] == array[m]){ //if these point to the same place
			//puts("\tignoring value");
			continue; //ignore it
		}
		//puts("\t1");
		if((array[i]->pos[dim] < array[m]->pos[dim]) && (i > m) ){ //if data is less than value and we are after the median
			//puts("\tmoving earlier");
			temp_p = array[i];//swap data and median
			array[i] = array[m];
			array[m] = temp_p;
			//restart from the earler point
			temp_i = i;
			i = m;
			m = temp_i;
		}
		//puts("\t2");
		if((array[i]->pos[dim] >= array[m]->pos[dim]) && (i < m)){ //if data is greater than or equal to value and we are before the median
			//puts("\tmoving later");
			temp_p = array[i];//swap data and median
			array[i] = array[m];
			array[m] = temp_p;
			//continue from earlier value, which is now also the median
			m = i;
		}
		//if(i==100) exit(0); //for debugging
	}
...
NODE* build_tree_array(PARTICLE **point_array, int N, int K, int depth){
	...
	int approx_limit = 1000000; //if more points than this, find approximate median
	int approx_N = 1000; //use this many points to find approximate median
	NODE *node = NULL; //node that we're finding
	PARTICLE **larray, **rarray;// array to send to left and right child
	...
	if( (N > 1) && (N < approx_limit) ){ // if 1<N<approx_limit, sort them and find median, this gives an exactly balanced tree
		...
		PARTICLE **approx_array; //pointer
		puts("N is larger than approx_limit, allocating space for approximation arrays");
		approx_array = (PARTICLE**) malloc( (int)sizeof(PARTICLE*)*approx_N ); //allocate stuff
...
double dist2(PARTICLE *a, double *b, int n_dim){
	...
	int i;
	for(i=0;i<n_dim;i++){
		dist2 += (a->pos[i]-b[i])*(a->pos[i]-b[i]); // dx^2+dy^2+dz^2=dist2
	}
	return(dist2);
}
...
void nearest_nn(NODE* node ,NODE** guess, double *g_dist2, PARTICLE* point, int n, int depth, int n_dim){
	...
	if(depth==0){ //this will only run right at the first call, for inits and checks
		...
		recompute_nn_dist(guess, g_dist2, point->pos, n, n_dim); //make sure any previous guesses are correct
	}
	if(node->val->ident != point->ident){ //if the node we're testing isn't the point we're looking around
		...
		if((present == -1)){ //if the node we're testing is not in the guess list already
			t_dist2 = dist2(node->val, point->pos, n_dim); //find it's distance
	...
	hyp_dist2 = dist2_d(&(node->val->pos[dim]), &(point->pos[dim]), 1); //find distance to hyper-plane
	if(point->pos[dim] < node->val->pos[dim]){
		first = node->left;
		second = node->right;
	}
	else{
...
void nearest_n(NODE* node, NODE** guess, PARTICLE* point, double *g_dist2, int depth, int n_dim){
	...
	if (point->ident != node->val->ident){ //if the node we're looking at isn't the point we're looking around
		t_dist2 = dist2(node->val, point->pos, n_dim);
		if(t_dist2 < *g_dist2){//if this point is closer
			*g_dist2 = t_dist2;//it becomes new guess
			*guess = node;
		}
	}
	hyp_dist2 = dist2_d(&(node->val->pos[dim]), &(point->pos[dim]), 1); //what is distance to hyper plane?
	if(point->pos[dim] < node->val->pos[dim]){
		first = node->left;
		second = node->right;
	}
...
void create_tree(NODE** root, PARTICLE** data, int N, int K){
	int i;
	//PARTICLE** storage=NULL; //storage array for 'build_tree_array' function
	//root = pointer to a pointer to a NODE structure
	//data = array in format { pointer_to_particle0, ptr2particle1, ptr2particle2,... ptr2particleN }
...
void tree_insert(NODE* node, PARTICLE* point, int K, int depth){
	...
	if(point->pos[dim] < node->val->pos[dim]){ //if should go left
	...
	if(point->pos[dim] >= node->val->pos[dim]){ //if should go right
...
int find_in_array(PARTICLE **point_array, PARTICLE *point, int N){
	...
	for(i=0;i<N;i++){
		...
		if(point->ident == point_array[i]->ident){ //if these are the same
			return(i);
		}
	}
